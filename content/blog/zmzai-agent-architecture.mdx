...
| **工具结果智能裁剪** | 部分落地：head/tail 确定性裁剪（70%/25%，带省略标记）已上线 | 失败日志按行剪裁（保错误行） |
| **compaction 重写历史** | 未解决 | 投影式（canonical 不动 + tail 预算 + 不切工具对） |
| **缓存计费断裂** | **已落地**：relay-agent-stream 解析 `cache_read_input_tokens`，usage 拆 input/output/cacheRead/cacheWrite 四路计费 | 折算精度随渠道持续校准 |
| **子代理无写路径隔离** | 未解决 | WritePathSet 声明式写路径 |

这些不是缺陷清单，而是**已经研究清楚、有明确借鉴源（Reasonix/dsh 真实源码）的演进路线**——前两行已经按这个路线落地，剩下的是下一批。
