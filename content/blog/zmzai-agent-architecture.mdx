---
title: "zmzai-agent 技术方案：一个 PI 驱动的云端多租户 Coding Agent 框架"
date: 2026-08-14
summary: "从分层架构、ReAct 循环、权限闸口、事件溯源到运行恢复——拆解一个生产级 coding agent 框架的完整设计。文末有配套的动手课程。"
tags: ["Agent 框架", "技术方案"]
draft: false
---
...
> 更新：2026-08-27 —— P0 工具面补齐收官：新增 §4.4 生命周期钩子、§6.7 git 工具集、§6.8 websearch 与 apply_patch、§6.9 交互式终端、§6.10 MCP 三传输客户端；§12 短板清单同步核销。
...
flowchart TB
    ...
    subgraph core["@zmzai/agent-framework（核心包）"]
        ...
        tools["Tools（内置 20+ / 本机 4<br/>+ MCP 外部工具）"]
...
### 4.4 生命周期钩子（`core/runtime/lifecycle.ts`）

`LifecycleHook` 四挂点，是插件生态的地基——审计、告警、自定义策略不再需要改 runner：

| 挂点 | 时机 | 能力 |
|---|---|---|
| `onRunStart` | 一次 run 开始 | 观测 |
| `onBeforeToolCall` | **权限评估之前**的第一道闸口 | 可 block，并把 reason 反馈给模型 |
| `onAfterToolCall` | 工具返回后 | 只读观测（被拦截的调用不触发） |
| `onRunEnd` | ok / aborted 终态 | 观测 |

容错语义：钩子抛错仅告警不中断运行；多个 block 类 hook 首个生效。钩子在权限评估**之前**执行，意味着它比权限规则更早——适合做「无论权限如何都禁止」的硬闸口。
...
> 框架最初内置八个 Workspace 工具（下表）；此后陆续补齐 webfetch、git×4、websearch、apply_patch、terminal×5（见 §6.7–§6.9），并新增 **MCP 外部工具注入通道**（见 §6.10）。另有 **4 个本机工具**由产品层经 `RunnerDeps.localTools` 注入（见 §6.6）——它们操作的是「用户自己的机器」，与 Workspace/沙箱完全不同的执行通道。
...
### 6.7 git 工具集（`core/tools/git.ts`）

四个工具把版本控制纳入 Agent 能力面：`git_status`（porcelain 结构化——分支、ahead/behind、暂存/未暂存/未跟踪三组）、`git_diff`（numstat 摘要，支持 staged 与单文件）、`git_log`（空仓库给友好提示）、`git_commit`（paths / addAll / staged 三种暂存语义，无变更时给引导性报错而非模糊失败）。

关键决策：git 工具**直接写真实仓库**而非沙箱快照副本。因此权限拆成两类——`git_read` 默认 allow，`git_write` 默认 ask 且可「总是允许」沉淀为规则（按仓库粒度）。

### 6.8 websearch 与 apply_patch（`core/tools/websearch.ts` / `patch.ts`）

**websearch** 是一个可测试设计样本：`createWebSearchTool` 注入 fetch/env——配置 `TAVILY_API_KEY` / `SERPER_API_KEY` 走对应 API，缺省回退 DuckDuckGo Lite（uddg 重定向还原，零 key 零依赖）。结果归一化为 title/url/snippet；权限复用 webfetch 分类，以 `search:` 前缀区分。

**apply_patch** 解析统一 diff（多文件多 hunk），采用**两阶段应用**：全量试算 → 逐文件落盘，任一失败整体不变更（不留半套补丁）。±300 行漂移容差应对模型行号偏差；新建文件走 `--- /dev/null`；删除明确报错。落盘走 workspace 门面，出不可变版本与 diff（`emitFileEdited`）。权限 = edit，按补丁受影响路径评估。

### 6.9 交互式终端（`core/tools/terminal.ts`）

五个工具：start / read / write / kill / list。`TerminalManager` 每会话一个环形缓冲 + 绝对游标续读 + 头部滚动丢弃计数。

- **start 即返不阻塞**——交互式进程（dev server、watch 模式）不能让 run 卡死
- **write** 支持 y/n 应答与裸按键（Ctrl+C）
- **kill** SIGTERM → SIGKILL 兜底
- **read** 返回终态退出码，防止模型对已退出进程空轮询

后端双模（`adapters/terminal-backend.ts`）：同步 `createRequire` 探测 node-pty（框架 optionalDependencies）→ 真 PTY；不可用降级管道模式（`sh -c` detached 进程组、负 pid 组杀）。一个实测细节：node-pty 的 `onExit` 会把普通退出的 `signal` 报成 0，必须归一化为「无信号」——否则每次 `echo` 都被标成 killed。

权限：新增 `terminal` 分类默认 ask（本机直接起进程，位阶高于沙箱 bash），always 规则按命令前缀沉淀。

### 6.10 MCP 客户端：外部工具注入通道（`core/mcp/`）

框架级 MCP 客户端，三种传输按 `spec.type` 分派：

| 传输 | 实现 | 细节 |
|---|---|---|
| stdio | NDJSON JSON-RPC 2.0 子进程 | 超时/进程退出时未决请求全部拒绝不悬挂；SIGTERM→SIGKILL 干净回收 |
| streamable-http | 单端点 POST | 应答兼容 `application/json` 与 `text/event-stream` 帧；`mcp-session-id` 捕获回传；notifications 走 202 |
| sse | GET 常开事件流承载数据 | endpoint 握手提示兼容裸路径 / JSON 字符串两种形态 |

装配语义：`startMcpServers` 批量启动，**单个 server 失败不影响其余**。工具命名空间化为 `mcp__<server>__<tool>`，权限映射 `mcp/<server>/<tool>`，`always` 支持按 `<server>/*` 整仓放行；isError 抛错接入既有循环防护（§12 第一行）。

类型系统上新增 `ExternalToolDef`：参数是**原生 JSON Schema**（MCP 上游不给 zod），ToolDef 家族扩展为 `AnyToolDef` 联合，runner 的 tools/localTools 都接受；edit 循环防护按 zod 变体收窄（外部工具不参与语义签名）。`localTools` 每次 run 重读——外部工具支持**运行期变更**（server 中途上下线不用重启会话）。
...
当上下文逼近模型窗口时，把旧历史压成摘要（`core/runtime/compaction.ts`）。2026-08-27 起改为**投影式**（进阶课第 6 期设计）：canonical 历史不可变，压缩只是投影——闭包持有 `{anchor, summary}` 两个状态，模型每次看到的永远是 `[摘要消息, ...messages.slice(anchor)]`。

```
transformContext（PI 每次请求前调用）
  ├─ projected = summary ? [摘要消息, ...messages.slice(anchor)] : messages
  ├─ estimateTokens(projected) + reserve < contextWindow? → 原样返回投影
  ├─ 滞回带：已有摘要但尾部增量 < 摘要体量的一半 → 不再重摘（防抖动）
  ├─ 否则增量压缩：
  │    fold  = messages[anchor : -keepRecent]   （keepRecent 默认 8）
  │    摘要提示词 = fold + 【已有摘要】旧摘要 + 压缩指令（增量续写，不是重启）
  │    膨胀拒绝：新摘要 ≥ 被替代内容（fold + 旧摘要）→ 作废，失败记忆
  │    anchor 前移 → return [摘要消息(timestamp 固定为 0), ...slice(anchor)]
  └─ 压缩失败 → 降级为当前投影（不硬失败，本轮不再重试）
```

压缩发生时，在最近一条 assistant message 上写一个 `compaction` Part 标记边界。**完整历史始终持久化在 store 里**，投影只改变模型可见上下文。摘要消息固定 `timestamp: 0`，两次压缩之间投影前缀逐字节稳定——provider 的前缀 prompt cache 可以持续命中。

### 已知局限（诚实说明）

- 投影状态（anchor/summary）在闭包里，每次 runLoop 重建——跨 run 需要重摘一次；跨 run 持久化留待后续
- 只在「超窗」时整体触发，没有按工具结果大小的细粒度压缩
- 摘要质量依赖小模型，没有并发锁 / 稳定性检查
...
| 短板 | 现状 | 改进方向 |
|---|---|---|
| **调用风暴防护** | **已落地**（P3 循环防护，`loop-guard.ts`）：storm 断路器（tool+归一化错误签名，阈值 3 注入"改策略"）+ blocked streak + edit 重复失败守卫（重试前先复查文件状态） | — |
| **工具结果智能裁剪** | **已落地**（2026-08-27）：head/tail 确定性裁剪（70%/25%，带省略标记）+ 失败日志按行剪裁（只留错误行 + 前后上下文、区间合并插省略标记、8000 字符上限、无错误行降级 head+tail），按结果状态 failed/succeeded 分流 | — |
| **compaction 重写历史** | **已落地**（2026-08-27 投影式改造）：canonical 不可变，投影 = [摘要?, slice(anchor)]；增量摘要续写、滞回带防抖、摘要消息 timestamp 固定为 0 保前缀缓存命中 | 跨 run 投影状态持久化 |
| **缓存计费断裂** | **已落地**：relay-agent-stream 解析 `cache_read_input_tokens`，usage 拆 input/output/cacheRead/cacheWrite 四路计费 | 折算精度随渠道持续校准 |
| **子代理无写路径隔离** | **已落地**（2026-08-27 WritePathSet）：`AgentInfo.writePaths` 声明式白名单，权限层 allow+全局 deny 兕底（LAST match wins）+ workspace 门面结构性抛错双层防御；opt-in，未声明的 preset 行为不变 | bash 沙箱侧的同粒度圈禁 |

这些不是缺陷清单，而是**已经研究清楚、有明确借鉴源（Reasonix/dsh 真实源码）的演进路线**——主表五项已全部落地（2026-08-27），剩余的是下一批细粒度方向（跨 run 摘要持久化、沙箱侧写路径圈禁等）。
...
builtinTools: ToolDef[]   // read/glob/grep/write/edit/todo/bash/task
                          // + webfetch / git×4 / websearch / apply_patch / terminal×5（§6.7–§6.9）
                          // 本机工具（local_fs_read 等 4 个）由产品层经 RunnerDeps.localTools 注入，见 §6.6
                          // MCP 外部工具经 ExternalToolDef（原生 JSON Schema）注入，见 §6.10
adaptTool(def, ctx): AgentTool   // 框架 ToolDef → PI AgentTool（接受 AnyToolDef 联合）
...
这篇文章讲的是「为什么这么设计」。如果你想把这套架构**亲手敲一遍**——从数据模型、工具系统、流式 LLM 调用、ReAct 循环，到权限闸口、事件流、上下文压缩——我准备了一个 **8 期动手实现型课程**：

每一期你跟着敲出一个能跑的模块（约 1000 行 TypeScript），最终拼成一个能读文件、改文件、跑命令、要权限、压缩上下文的 mini coding agent。学完你能读懂任何生产级 agent 框架（zmzai-agent / OpenCode / Reasonix）的核心代码。

👉 **[从零造一个 Coding Agent 框架 · 8 期课程](/courses)**（免费，全部 8 章可读）

两门姊妹篇同样免费：**[Agent 健壮性进阶篇 · 8 期](/courses)**——把上面 §12 的短板清单逐个设计落地（裁剪、去重、断路器、修复管线、投影压缩、子代理隔离、运行恢复）；**[Harness 拆解课 · 10 期](/courses)**——横评 Codex / Claude Code / OpenCode / Gemini CLI / Pi / DeepSeek 七家的真实 harness，最后一期把精华装回 zmzai-agent。
