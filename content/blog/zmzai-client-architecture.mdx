...
```mermaid
flowchart LR
    A["zmzai-agent<br/>工具循环"] -->|"local-tool"| B["zmzai-relay<br/>控制面（鉴权 + 用户校验）"]
    B -->|"POST /v1/users/:userId/tool"| C["zmzai-bridge<br/>路由 + 鉴权"]
    C <-->|"反向隧道 WS<br/>hello / welcome / tool_request / tool_result"| D["zmzai-client<br/>本机 Electron"]
    D -->|"fs / shell / notify"| E["用户本机<br/>审批 + 审计"]
```
